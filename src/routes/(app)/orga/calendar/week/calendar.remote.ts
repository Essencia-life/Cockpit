...
export const getEvents = query(async () => {
	...
	const res = await calendar.events.list({
		...
		singleEvents: true,
		orderBy: 'startTime',
		timeMin: new Date(today.getFullYear(), today.getMonth(), today.getDate() - today.getDay() + 1).toISOString(),
		timeMax: new Date(today.getFullYear(), today.getMonth(), today.getDate() - today.getDay() + 8).toISOString(),
		privateExtendedProperty: ['source=week-plan']
	});

	return res.data.items;
});

const saveEvent = async ({ eventId, requestBody }: { eventId?: string; requestBody: any }) => {
...
const twoInts = v.pipe(v.array(int), v.length(2));
...
export const saveMorningPractise = form(
	v.object({
		day: v.string(),
		practise: v.pipe(v.string(), v.nonEmpty()),
		facilitator: twoInts
	}),
	async ({ day: dayStr, ...data }) => {
		const day = new Date(dayStr);

		await saveEvent({
			// TODO pass eventId
			requestBody: {
				summary: '🤸️ Morning Practise',
				description: `${data.practise} with ${telegramUser(data.facilitator[0])}${data.facilitator[1] ? ` & ${telegramUser(data.facilitator[1])}` : ''}`,
				visibility: 'private',
				location: 'Shala',
				start: {
					dateTime: new Date(day.getFullYear(), day.getMonth(), day.getDate(), 8).toISOString(),
					timeZone: 'Europe/Lisbon'
				},
				end: {
					dateTime: new Date(day.getFullYear(), day.getMonth(), day.getDate(), 9).toISOString(),
					timeZone: 'Europe/Lisbon'
				},
				extendedProperties: {
					private: {
...
export const saveLunch = form(
	...
	async ({ day: dayStr, ...data }) => {
		...
		await saveEvent({
			...
			requestBody: {
				summary: 'Lunch',
				description: `🧑‍🍳 ${telegramUser(data.chef[0])}${data.chef[1] ? ` & ${telegramUser(data.chef[1])}` : ''} 🧽 ${telegramUser(data.cleaner)}`,
				visibility: 'private',
				location: 'Fun Kitchen',
				start: {
					dateTime: new Date(day.getFullYear(), day.getMonth(), day.getDate(), 14).toISOString(),
					timeZone: 'Europe/Lisbon'
				},
				end: {
					dateTime: new Date(day.getFullYear(), day.getMonth(), day.getDate(), 15).toISOString(),
					timeZone: 'Europe/Lisbon'
				},
				extendedProperties: {
					private: {
