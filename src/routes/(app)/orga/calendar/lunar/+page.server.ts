...
async function eventsByView(
	...
	return icalEvents
		.filter((event) => {
			const between =
				event.start.getTime() >= start.getTime() && event.start.getTime() <= end.getTime();
...
export const load: PageServerLoad = async ({ url }) => {
	...
	const events = await eventsByView(
		moonPhases.lastNewMoon.date,
		moonPhases.nextNewMoon.date,
		EVENTS_ICAL
	);
	const retreats = await eventsByView(
		moonPhases.lastNewMoon.date,
		moonPhases.nextNewMoon.date,
		RETREATS_ICAL
	);
	const community = await eventsByView(
		moonPhases.lastNewMoon.date,
		moonPhases.nextNewMoon.date,
		COMMUNITY_ICAL,
		true
